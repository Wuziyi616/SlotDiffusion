...
def miou_metric(gt_mask, pred_mask):
    ...
    ious = [
        hungarian_miou(gt_mask[i], pred_mask[i], ignore_background=False)
...
def fmiou_metric(gt_mask, pred_mask):
    """both mask: [B, H, W], both are seg_masks after argmax."""
    assert 'int' in str(gt_mask.dtype)
    assert 'int' in str(pred_mask.dtype)
    gt_mask, pred_mask = gt_mask.flatten(1, 2), pred_mask.flatten(1, 2)
    ious = [
        hungarian_miou(gt_mask[i], pred_mask[i], ignore_background=True)
        for i in range(gt_mask.shape[0])
    ]
    if all(np.isnan(iou) for iou in ious):
        print('WARNING: all miou in a batch are nan')
...
def mbo_metric(gt_mask, pred_mask):
    """both mask: [B, H, W], both are seg_masks after argmax."""
    assert 'int' in str(gt_mask.dtype)
    assert 'int' in str(pred_mask.dtype)
    gt_mask, pred_mask = gt_mask.flatten(1, 2), pred_mask.flatten(1, 2)
    mbos = [
        mean_best_overlap(gt_mask[i], pred_mask[i])
        for i in range(gt_mask.shape[0])
    ]
