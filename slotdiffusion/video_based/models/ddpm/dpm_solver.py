...
def model_wrapper(
    ...
    def noise_pred_fn(x, t_continuous, cond=None):
        ...
        if cond is None:
            ...
            output = model(x, t_input, context=cond, **model_kwargs)
        if model_type == "noise":
            return output
    ...
    def quantize_fn(x):
        """
        Apply 1st stage VQVAE model to quantize the feature map.
        """
        return model.vae.quantize(x)

    def model_fn(x, t_continuous=None, quantize=False):
        """
        The noise predicition model function that is used for DPM-Solver.
        """
        if quantize:
            assert t_continuous is None
...
class DPM_Solver:
    def __init__(
        ...
        Args:
            ...
            dynamic_thresholding_ratio: A `float`. The ratio for dynamic thresholding (see Imagen[1] for details).
                Valid only when use `dpmsolver++` and `correcting_x0_fn="dynamic_thresholding"`.
            vq_denoised: A `bool`. Use 1st stage VQVAE to denoise the predicted x0.
        ...
        self.vq_denoised = vq_denoised
        assert (self.correcting_x0_fn is None) or (not self.vq_denoised), \
            'dynamic thresholding is image space DM, vq_denoised is LDM'
    ...
    def data_prediction_fn(self, x, t):
        ...
        if self.vq_denoised:
            x0 = self.model(x0, None, quantize=True)
        return x0
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        method='multistep', lower_order_final=True, denoise_to_zero=False, solver_type='dpmsolver',
        atol=0.0078, rtol=0.05, return_intermediate=False, verbose=False,
    ):
        """
        ...
        Args:
            ...
            verbose: A `bool`. Whether to print the time used for sampling.
        Returns:
        ...
        if verbose:
            print(f'DPM-Solver sampling time {time.time() - start_t:.3f}s')
