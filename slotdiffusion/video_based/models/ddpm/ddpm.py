...
class DDPM(nn.Module):
    ...
    def __init__(
        ...
        self.log_every_t = diffusion_dict.pop('log_every_t', 200)
    ...
    def register_schedule(
        ...
        assert pred_target in ['eps', 'x0', 'v']
    ...
    def forward(self, x, t, **kwargs):
        """Forward pass."""
        # U-Net forward pass for one-step denoising
    ...
    def _get_rows_from_list(self, samples):
        """Input: [N, T, C, H, W]."""
        N, T = samples.shape[:2]
        denoise_grid = torch.stack([
            make_grid(samples[:, i], nrow=N, pad_value=-1.) for i in range(T)
        ])  # [T, C, H, N*W]
        return denoise_grid
        ...
        **kwargs,
        ...
        with self.ema_scope("Plotting"):
            ret = self.generate_imgs(
                batch_size=B, ret_intermed=ret_intermed, **kwargs)
