...
from .slate_utils import linear
...
class SLATETransformerDecoder(nn.Module):
    ...
    def generate(self, slots, steps, sample=False, temperature=1.0, verbose=False):
        ...
        all_logits = []
        idx_lst = tqdm.trange(steps) if verbose else range(steps)
        for _ in idx_lst:
            # predict one step
